    ...
    while (len > 0)
    {
        ...
        if (prot_changed)
        {
            ...
            if (!VirtualProtect((void*)dest, this_len, prot_prev, &dummy)) {
                Plugin.Log("__page_write() could not restore original permissions for ptr {:X}", reinterpret_cast<address_t>(dest));
            }
        }
    ...
    while (len > 0)
    {
        ...
        if (prot_changed)
        {
            ...
            if (!VirtualProtect((void*)src, this_len, prot_prev, &dummy)) {
                Plugin.Log("__page_read() could not restore original permissions for ptr {:X}", reinterpret_cast<address_t>(src));
            }
        }
